...
static FLASH_PART get_erase_sector(uint32_t addr,uint32_t len)
{
	...
	else if(addr>=REGION_FLASH_SECTOR4&&(addr+len-1)<REGION_FLASH_SECTOR5 ){
		return PART_DISPLAY;
	}
...
uint32_t VR_flash_erase(uint32_t addr, uint32_t length)
{
    ...
    HAL_FLASH_Unlock();
    /* Clear ERR flags */
    __HAL_FLASH_CLEAR_FLAG(  FLASH_SR_WRPRTERR | FLASH_SR_PGAERR | FLASH_SR_PGERR );
    ...
    switch(curr_sec){
    	...
    	case PART_MISC_DATA:
      	...
      	case PART_LOG:
			...
			break;
      	case PART_DISPLAY:
			EraseInfo.Sector= FLASH_SECTOR_4;
			EraseInfo.NbSectors = 1;
			break;
      	case PART_FOTA:
...
uint32_t flash_ob_status(void)
{
    ...
    /* Print the status */
    printf("  WRPSector 0x%x,  RDPLevel 0x%x, SPR mode 0x%x \r\n",\
