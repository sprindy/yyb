...
/* sprindy: no HAL_DBG_UART_Transmit() in stm32f1xx_hal_uart.c */
PUTCHAR_PROTOTYPE
{
	...
	if (ch != '\n') {
		/* HAL_DBG_UART_Transmit(&huart2, (uint8_t *)&ch, 1, 0xFFFF); */
		HAL_UART_Transmit(&huart2, (uint8_t *)&ch, 1, 0xFFFF);
		goto done;
	}

	if (prev_ch != '\r') {
		/* HAL_DBG_UART_Transmit(&huart2, (uint8_t *)"\r", 1, 0xFFFF); */
		/* HAL_DBG_UART_Transmit(&huart2, (uint8_t *)&ch, 1, 0xFFFF); */
